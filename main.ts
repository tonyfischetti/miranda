...
export default class MyPlugin extends Plugin {
  ...
  async onload() {
    ...
    const addDefaultDateIfNotThere = (taskString: string): string => {
      if (taskString.match(/\[due:: \d/)) return taskString;
      return `${taskString} [due:: ${getYYYYMMDD()}]`;
    };

    const addTaskToFile = async (afile: string) => {
      const tasksApi = this.app.plugins.plugins['obsidian-tasks-plugin'].apiV1;
      let taskLine = await tasksApi.createTaskLineModal();
      taskLine = addDefaultDateIfNotThere(taskLine);
    ...
    this.addCommand({
      ...
      callback: () => {
        getDailyNote().
          then(_ => { console.log({hey: _}); return _; }).
          then(addTaskToFile);
      }
    });
